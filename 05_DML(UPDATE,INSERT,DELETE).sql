...
-- DML -- 데이터 조작 언어
-- 테이블에 값을 INSERT 하거나, UPDATE 하거나, DELETE 하는 구문

-- * 주의 사항 * COMMIT, ROLLBACK 하지 말기
...
-- INSERT 하고자 하는 컬럼이 모든 컬럼 인 경우 컬럼명 생략 가능
-- 순서는 꼭 지켜서 VALUES에 값을 넣기


INSERT INTO EMPLOYEE2
VALUES(900, '장채현', '901123-1080503', 'jang_ch@kh.or.kr', '01055569512', 'D1', 'J8', 'S3', 4300000, 0.2, '200', SYSDATE, NULL, 'N');
...
ROLLBACK; -- 이전 시점으로 돌아갈 때 사용

DELETE FROM EMPLOYEE2
WHERE EMP_ID = 900; -- 전부 다 삭제되는 것을 볼 수 있다. 
-- 왜냐하면, 조건이 900번이라서 900번 전부 다 삭제

COMMIT; --COMMIT 하는 순간 ROLLBACK을 해도 돌아가지 않는다.
		-- 확실한 데이터만 COMMIT 해야한다.

--------------------------------------------------------------------------------

-- 내가 선택한 컬럼만 값 넣기
-- 2) INSERT INTO 테이블명
--    테이블명(컬럼명1, 컬럼명2, 컬럼명3....) VALUES(데이터1, 데이터2, 데이터3....);
--    컬럼명1 = 데이터1, 컬럼명2 = 데이터2, 컬럼명3 = 데이터3
--    테이블에 내가 선택한 컬럼에 대한 값만 INSERT 할 때 사용한다
--    선택 안된 컬럼은 값이 NULL이 들어가게 된다. (DEFAULT 값이 존재 시 DEFAULT 값으로 삽입됨)


INSERT INTO EMPLOYEE2 (EMP_ID, EMP_NAME, EMP_NO, EMAIL, PHONE, DEPT_CODE, JOB_CODE, SAL_LEVEL, SALARY)
VALUES('900','장채현','900910-2323231', 'TEST@GMAIL.COM', '01012341234', 'D1', 'J7', 'S3', 4500000);
...
--> SELECT 조회 결과의 데이터 타입, 컬럼 갯수가
-- INSERT 하려는 테이블의 컬럼과 일치해야한다.
...
-- DEPARTMENT2 테이블에서 DEPT_ID가 D9인 행의 DEPT_TITLE을 '전략기획팀'으로 바꾸기


UPDATE DEPARTMENT2
SET DEPT_TITLE = '전략기획팀'
WHERE DEPT_ID = 'D9'; -- D9은 전략기획팀으로 수정
...
FROM EMPLOYEE2
WHERE EMP_NAME IN ('유재식', '방명수'); -- 둘다 3,400,000 그리고 0.2로 변경
...
AS SELECT * FROM EMPLOYEE -- EMPLOYEE를 본떠 만들었으나 JOB_CODE 가 J4인 애만 본떠 만듦
WHERE JOB_CODE = 'J4';
...
-- * 주의 사항 * ROLLBACK을 통해서 복구를 할 수 없다.
...
SELECT *
FROM EMPLOYEE3; -- ROLLBACK 후 복구확인을 했는데 복구 안됨을 확인함.
